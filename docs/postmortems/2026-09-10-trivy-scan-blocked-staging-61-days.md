...
| 4 | Add a weekly job that fails when any `.trivyignore.yaml` entry is within 14 days of `expired_at`, so expiry surfaces as a warning before it becomes a build failure — **DONE 2026-09-15**: `.github/workflows/trivyignore-expiry.yml` + `script/check-trivyignore-expiry.mjs`, weekly + manual dispatch, alerts via the existing `ci-failure-notify.yml` Telegram path | detect | **P1** | [role] | [#729](https://github.com/ilv78/Art-World-Hub/issues/729) ✅ |
