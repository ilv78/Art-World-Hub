...
## 2026-05-11 — Move OG card endpoint out of `/api/*` (#593)

- Route moved from `/api/og/:type/:id.jpg` → `/og/:type/:id.jpg`. Other major scrapers (WhatsApp, LinkedIn, Telegram, iOS) fetched the original path fine, but Facebook's scraper silently skips og:image URLs whose path starts with `/api/`. Cross-checked against rotated nginx access logs: zero historical FB hits to `/api/og/...`, yet FB scraped the parent HTML page (with a valid og:image meta tag pointing at it) hundreds of times. Hypothesis confirmed on FB Sharing Debugger after the path move.
- On-disk cache path `uploads/og-cards/<type>-<id>.jpg` is unchanged — only the public-facing route prefix moves.
- `server/meta.ts` `ogCardUrl()` updated; tests + spec narrative + this changelog entry updated.
- Old `/api/og/...` URLs are not redirected: external caches (FB/X) will pick up the new path on next scrape, and there's no in-app feature that bookmarks OG-card URLs.

## 2026-05-04 — Branded OG card images (#577)

- New endpoint `GET /api/og/:type/:id.jpg` (type ∈ artwork|blog|exhibition|artist) — **moved to `/og/...` in #593, see entry above**. Renders a 1200×630 branded JPEG: source image as a blurred + lightly-darkened background, title (Playfair Display Bold) + subtitle (Inter Regular) on the upper-left, full-width brand-orange divider near the bottom, and the in-app brand mark (V diamond + `vernis9.art` wordmark) below it.
...
- `server/meta.ts`: `ogImage` on the four item branches (artwork, blog, exhibition, artist; plus the `?artwork=<slug>` short-circuit) now points at `/og/<type>/<id>.jpg?v=<8charHash>` (originally `/api/og/...`, moved in #593). Hash is computed from title|subtitle|sourceImageUrl so any displayed-field change busts external caches. **Static routes (`/`, `/store`, `/gallery`, etc.) are intentionally untouched — only items shared via the in-app share button get the branded card.**
