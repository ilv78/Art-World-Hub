...
| **artworks** | `shared/schema.ts` | id, title, description, imageUrl, artistId (FK→artists), price (decimal), medium, dimensions, year, isPublished, isForSale, isInGallery, isReadyForExhibition, exhibitionOrder, category | Art pieces. `isPublished` gates `isForSale`/`isInGallery`/`isReadyForExhibition` — drafts clamp those flags to false server-side. Drafts are excluded from public GETs (owner sees own drafts on authenticated requests) |
