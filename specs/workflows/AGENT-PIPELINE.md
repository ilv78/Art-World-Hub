...
| PR touching `shared/schema.ts` or `migrations/` goes `DIRTY` against `main` mid-run | Rebase, regenerate the migration number and Drizzle snapshot with `drizzle-kit generate`, re-run `npm run check` (a coexisting schema change can break a fixture your own diff never touched), re-push. Not an escalation (policy §6). | — |
| **Required checks missing from the list entirely** | The PR is conflicted. Rebase — do not investigate the checks. | — |

**The table above covers a run hitting `DIRTY` on its own PR, mid-run, while an agent is
still present to fix it.** A schema-touching PR can also go `DIRTY` *after* its authoring
run has already ended — a later PR merges underneath it — with nobody watching. Auto-merge
(`auto-merge.yml`) already re-updates any `agent-review`/`autorelease` PR that goes
`BEHIND` on every push to `main` (#793); it does not yet do the equivalent for `DIRTY`, so
a PR in that state today sits blocked with no escalation until a human notices. A drafted
extension — flag a `DIRTY` `agent-review` PR touching `shared/schema.ts` or `migrations/`
with a comment and `agent-stuck`, reusing the existing silent-escalation path — is recorded
in `specs/decisions/log/2026-09-18-schema-dirty-conflict-handling.md`, blocked on the same
`.github/workflows/` PAT restriction as everything else in this section, pending a human
applying it directly. (#820)
