...
| Trivy scan | Runs Trivy against the locally-built image and writes `trivy-report.json` (CRITICAL/HIGH, unfixed ignored, suppressions from `.trivyignore.yaml` — empty since #739). `exit-code: 0` — the scanner reports, it does not decide |
...
| 2026-09-11 | Added `pr-contract.yml` — the mechanical half of the reviewer step in the autonomy pipeline (#744). Checks that a PR states its issue, has a `## Verification` section, says what CI does and does not cover (code PRs only), and declares any deviation from the issue. Enforces declaration, not truth. Deliberately does **not** duplicate the Documentation Agent's consistency rules (C-001, C-003, ST-001/004/005). Not a required status check initially — the lesson from `Gated Path Review` was to prove a gate stable on real PRs before making it mandatory. ([#744](https://github.com/ilv78/Art-World-Hub/issues/744)) |
